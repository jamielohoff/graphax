...
def safe_preeliminations_gpu(edges: chex.Array, 
    ...
    def update_edges(carry, vertex):
        ...
        col = _edges[:, vertex-1]
        row_flag = jnp.sum(jnp.where(row > 0, 1, 0)) == 1
        col_flag = jnp.sum(jnp.where(col > 0, 1, 0)) == 1
        ...
        is_output_vertex = jnp.bool_(((vertex == output_vertices).sum()))
...
def compress_graph(edges: chex.Array, 
    ...
    for _ in range(1, num_intermediates+1):            
        ...
        if s1 and s2:           
            add_mask = jnp.where(output_vertices >= i, 1, 0)
            output_vertices -= add_mask     
    ...
    return edges, new_info, lax.slice_in_dim(output_vertices, 0, num_intermediates-num_removed_vertices), attn_mask
