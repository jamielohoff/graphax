...
def construct_random_graph(ninputs: int, 
    ...
    for vertex in range(-ninputs+1, nintermediates+noutputs):
        int_key, key = jrand.split(key, 2)
        num_edges = jrand.randint(int_key, (1,), minval=low, maxval=high)[0]
        other_vertices = jrand.randint(int_key, (num_edges,), 
                                    minval=vertex+1, 
                                    maxval=nintermediates+noutputs)  
        for other_vertex in other_vertices:
            gs = add_edge(gs, (vertex, other_vertex), 1., gs.info)
    return gs
